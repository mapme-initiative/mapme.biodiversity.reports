...
author: 
  - name: "Johannes Schielein & Andreas Petutschnig"
...
# Add GEOMETRY_TYPE column if not present
if (!"GEOMETRY_TYPE" %in% names(wdpa_allPAs)) {
  wdpa_allPAs$GEOMETRY_TYPE <- st_geometry_type(wdpa_allPAs)
}
...
if (file.exists("../data/bolivia_burned_area_total_2000_2022.csv")) {
  ...
  names(burned_area_data)[1] <- "WDPAID"
...
wdpa_allPAs_lossdata <- wdpa_allPAs %>%
  ...
  filter(as.character(GEOMETRY_TYPE) != 'POINT') %>%
...
if (nrow(wdpa_kfw_treated) > 0 && "ORIG_NAME" %in% names(wdpa_kfw_treated)) {
  map <- map %>% addPolygons(
    data = wdpa_kfw_treated, opacity = 0.9, color = "#2b8cbe",
    group = "PAs KfW-supported", label = ~ORIG_NAME, weight = 1
  )
}

if (nrow(wdpa_nonSupportPA) > 0 && "ORIG_NAME" %in% names(wdpa_nonSupportPA)) {
  map <- map %>% addPolygons(
    data = wdpa_nonSupportPA, opacity = 0.9, color = "darkgrey",
    group = "PAs (Others)", label = ~ORIG_NAME, weight = 2
  )
}

if (nrow(bolivia_suggested_PAs) > 0 && "ORIG_NAME" %in% names(bolivia_suggested_PAs)) {
  map <- map %>% addPolygons(
...
map2 <- map2 %>%
  addCircleMarkers(
    ...
    group = "Burned Area (2000-2024)",
    radius = 8,
    popup = ~label_fire,
    stroke = FALSE,
    fillOpacity = 0.8
  ) %>%
  # Label markers temporarily disabled
  # addLabelOnlyMarkers(
  #   data = wdpa_allPAs_lossdata_subset,
  #   lng = ~X, lat = ~Y, label = ~ORIG_NAME, group = "Labels (PA Names)",
  #   labelOptions = labelOptions(noHide = TRUE, direction = 'top', textOnly = TRUE)
  ...
  addLayersControl(
    baseGroups = c("CartoDB", "OpenStreetMap", "Satellite", "Topography"),
    overlayGroups = c("PAs Analyzed", "PAs KfW-supported", "PAs (Others)",
                     "Labels (PA Names)", "Burned Area (2000-2022)",
                     "Forest Cover Loss (2001-2024)"),
    options = layersControlOptions(collapsed = FALSE)
  ) %>%
  hideGroup(c("PAs (Others)", "PAs KfW-supported", "Forest Cover Loss (2001-2024)",
              "Labels (PA Names)"))
