...
# MCD64A1 data available for 2015-2022 (complete tile coverage)
if (file.exists("../data/bolivia_burned_area_total_2015_2022.csv")) {
  burned_area_data <- read_csv(
    "../data/bolivia_burned_area_total_2015_2022.csv",
    show_col_types = FALSE
  )
...
wdpa_temp$label <- paste(
  ...
  "Burned Area (2015-2022):", format(round(wdpa_temp$total_burned_area_ha, 0), big.mark = ","), "ha",
    "</p>"
  )
...
map <- map %>%
  ...
  addLayersControl(
    ...
    overlayGroups = c("PAs KfW-supported", "PAs (Others)",
...
## Burned Area Analysis (2015-2022)
...
wdpa_allPAs_lossdata_subset$label_fire <- paste(
  ...
  "Burned Area (2015-2022):", format(round(wdpa_allPAs_lossdata_subset$total_burned_area_ha, 0), big.mark = ","), "ha",
    "</p>"
)
...
map2 <- map2 %>%
  ...
  addLegend(
    ...
    title = "Total Burned Area<br>(2015-2022)",
    opacity = 1
  ) %>%
  addLayersControl(
    baseGroups = c("CartoDB", "OpenStreetMap", "Satellite", "Topography"),
    overlayGroups = c("PAs KfW-supported", "PAs (Others)", "Burned Area (2015-2022)"),
    options = layersControlOptions(collapsed = FALSE)
  )
...
if (nrow(lollipop_data_fire) > 0) {
  ...
  p <- lollipop_data_fire %>%
    ...
    labs(
      x = "",
      y = "Total Burned Area in ha (2015-2022)",
      color = "KfW Project Number"
    ) +
    scale_y_continuous(labels = comma) +
    theme_minimal() +
...
# Load annual burned area data if it exists (2015-2022)
if (file.exists("../data/bolivia_burned_area_annual_2015_2022.csv")) {
  burned_annual <- read_csv(
    "../data/bolivia_burned_area_annual_2015_2022.csv",
    show_col_types = FALSE
  )
  ...
  # Convert wdpa_id to character and join names
  burned_annual <- burned_annual %>%
    mutate(wdpa_id = as.character(wdpa_id)) %>%
