...
# Create table data - prioritize KfW-supported areas first
table_data <- bolivia_suggested_PAs %>%
  st_drop_geometry() %>%
  mutate(WDPAID = as.character(WDPAID)) %>%
  left_join(forest_2001, by = "WDPAID") %>%
  left_join(forest_latest, by = "WDPAID") %>%
  left_join(burned_total, by = "WDPAID") %>%
  mutate(
...
::: {.callout-warning}
## Important Limitation: GFW Detection Bias in Dry Forest Ecosystems
...
The **Composite Threat Index** used in this analysis follows established approaches in conservation prioritization (Margules & Pressey, 2000; Wilson et al., 2005). The index combines three normalized indicators:
...
**1. Otuquis National Park** - Ranks among the highest-threat areas due to catastrophic fire events during the **2019-2020 Bolivian fire crisis**. According to reports from the Fundación Amigos de la Naturaleza (FAN), this Pantanal-Chaco transition zone experienced unprecedented fire activity that burned over 5 million hectares across eastern Bolivia in 2019 alone.
